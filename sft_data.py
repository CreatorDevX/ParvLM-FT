...
def format_smoltalk2(example: dict, tokenizer: AutoTokenizer) -> dict:
...
def format_input_output(example: dict, tokenizer: AutoTokenizer) -> dict:
    messages = [
...
def format_glm(example: dict, tokenizer: AutoTokenizer) -> dict:
...
def build_streaming_dataset(
    ...
    for cfg in configs:
        ...
        if cfg.formatter is not None:
            ds = ds.map(lambda x, f=cfg.formatter, t=tokenizer: f(x, t))
